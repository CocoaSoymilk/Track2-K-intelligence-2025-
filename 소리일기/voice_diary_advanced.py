...
import pytz, io, os, sys, json, base64, tempfile, hashlib, random, calendar, warnings
...
def get_librosa():
    try: import librosa; return librosa
...
@st.cache_resource(show_spinner=False)
def get_webrtcvad():
    try: import webrtcvad; return webrtcvad
    except Exception: return None
...
def show_disclaimer():
    if st.session_state.show_disclaimer:
        ...
        if c1.button("✅ 이해했습니다", type="primary"):
            st.session_state.show_disclaimer=False; st.rerun()
        if c2.button("📊 데모 데이터로 시작하기"):
            load_demo_data(); st.session_state.show_disclaimer=False; st.success("가상 데이터 로드됨!"); st.rerun()
...
def load_demo_data():
    ...
    for i,s in enumerate(scenarios):
        ...
        st.session_state.diary_entries.append({
            ...
            "text": s["t"],
            "analysis": {"emotions":s["e"],"stress_level":s["S"],"energy_level":s["E"],"mood_score":s["M"],
                         "summary":f"{s['tone']} 상태의 하루.","keywords":[],"tone":s["tone"],
                         "confidence":float(np.round(random.uniform(0.7,0.9),2))},
            "audio_data": None,
            "mental_state": {"state":"안정/회복" if s["S"]<40 else ("고스트레스" if s["S"]>70 else "중립"),
...
class VoiceFeatureExtractor:
    ...
    def extract(self, audio_bytes:bytes):
        if not librosa: return self._default()
        try:
            ...
            if parselmouth and sf:
                ...
                try:
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as t2:
                        sf.write(t2, y, sr, subtype="PCM_16", format="WAV"); tmp=t2.name
                    snd=parselmouth.Sound(tmp)
                    harm=snd.to_harmonicity_cc(); hnr=float(np.nan_to_num(harm.values.mean(), nan=15.0))
            ...
            return {"duration_sec":float(dur),"pitch_mean":float(pitch_mean),"pitch_variation":float(pitch_var),
                    "energy_mean":float(energy_mean),"energy_max":float(energy_max),"tempo":float(tempo),
                    "zcr_mean":float(zcr_mean),"spectral_centroid_mean":float(spectral_centroid_mean),
                    "hnr":float(hnr),"jitter":float(jitter)}
        except Exception:
            return self._default()
    def _default(self):
        return {"duration_sec":0.0,"pitch_mean":150.0,"pitch_variation":0.13,"energy_mean":0.08,"energy_max":0.12,
                "tempo":110.0,"zcr_mean":0.10,"spectral_centroid_mean":2000.0,"hnr":15.0,"jitter":0.012}
...
def safe_json_parse(s:str)->dict:
    ...
    try: return json.loads(t)
        ...
        try:
            ...
            if i>=0 and j>i: return json.loads(t[i:j])
...
def analyze_text_with_llm(text:str, voice_cues_for_prompt=None)->dict:
    if not openai_client or not text.strip(): return analyze_text_simulation(text)
    cues=""
    ...
    def _call():
        return openai_client.chat.completions.create(
            ...
            messages=[{"role":"system","content":TEXT_ANALYZER_SYSTEM},{"role":"user","content":user_prompt}]
        )
    resp=call_llm_safely(_call)
    if not resp or not resp.choices or not resp.choices[0].message.content: return analyze_text_simulation(text)
...
def combine_text_and_voice(tres, voice=None):
    if not voice or "voice_cues" not in voice: return tres
    cues=voice["voice_cues"]; q=float(cues.get("quality",0.5))
    base=0.25*q
    if q<0.3: base=0.0
    tone=tres.get("tone","중립적")
    if tone=="긍정적": base*=0.6
    elif tone=="부정적": base*=0.9
    MAX_DS,MAX_DE,MAX_DM=12,12,10
    stress=tres.get("stress_level",30); energy=tres.get("energy_level",50); mood=tres.get("mood_score",0)
    ...
    ds=float(np.clip(ds,-MAX_DS,MAX_DS)); de=float(np.clip(de,-MAX_DE,MAX_DE)); dm=float(np.clip(dm,-MAX_DM,MAX_DM))
    out=dict(tres)
...
def preprocess_audio_for_asr(audio_bytes:bytes, target_sr=16000)->bytes:
    if not librosa or not sf: return audio_bytes
    y, sr = librosa.load(io.BytesIO(audio_bytes), sr=target_sr, mono=True)
    if y.size==0: return audio_bytes
    try: y = librosa.effects.preemphasis(y, coef=0.97)
    except Exception: pass
    if webrtcvad:
        try:
            vad=webrtcvad.Vad(2); frame_ms=30; frame_len=int(target_sr*frame_ms/1000)
    ...
    rms=float(np.sqrt(np.mean(y**2))+1e-8); gain=float(np.clip(0.08/rms,0.5,4.0)); y=np.clip(y*gain,-1,1)
    buf=io.BytesIO(); sf.write(buf, y, target_sr, subtype="PCM_16", format="WAV"); return buf.getvalue()
...
def build_coach_payload(text, combined, kb_ctx):
    ...
    return {
        "text": text,
        "signal": {"stress":combined.get("stress_level",0),"energy":combined.get("energy_level",0),"mood":combined.get("mood_score",0),
                   "voice":{"arousal":int(cues.get("arousal",50)),"tension":int(cues.get("tension",50)),
                            "stability":int(cues.get("stability",50)),"quality":float(cues.get("quality",0.5))}},
        "kb_context": kb_ctx,
        "constraints": {"max_items":4,"no_medical":True,"language":"ko-KR"}
    }

def coach_with_rag(text, combined, kb_ctx)->dict:
    if not openai_client: return assess_mental_state(text, combined)
    payload=build_coach_payload(text, combined, kb_ctx)
    def _call():
        return openai_client.chat.completions.create(
            model="gpt-4o", temperature=0.4, max_tokens=650,
            response_format={"type":"json_object"},
            messages=[{"role":"system","content":COACH_SYSTEM},{"role":"user","content":json.dumps(payload, ensure_ascii=False)}]
        )
    resp=call_llm_safely(_call)
    if not resp or not resp.choices or not resp.choices[0].message.content:
        return assess_mental_state(text, combined)
    data=safe_json_parse(resp.choices[0].message.content)
    if not data: return assess_mental_state(text, combined)
...
def extract_positive_events(text:str)->list[str]:
    t=text.lower()
    pairs=[("좋았","오늘 좋았던 점"),("행복","행복한 순간"),("고마","감사한 일"),
...
def assess_mental_state(text, combined)->dict:
    ...
    return {"state":state,"summary":summary,"positives":positives,"recommendations":recs,"motivation":mot,
    ...
    for root in search_roots:
        ...
        for dirpath, _, files in os.walk(root_path):
            if filename_nfc in files:
                ...
                try:
                    if p.exists(): results.append(str(p))
                except Exception:
                    pass
    if not results:
...
def tfidf_matrix(chunks:list[str]):
    ...
    vocab={}
    for tks in docs:
        for t in set(tks): vocab[t]=vocab.get(t,0)+1
    N=len(chunks); idf={t: np.log((N+1)/(df+1))+1.0 for t,df in vocab.items()}
    rows=[]
    for tks in docs:
        tf={}
        for t in tks: tf[t]=tf.get(t,0)+1
...
def build_kb_index(paths:list[str]):
    ...
    for p in paths:
        ...
        for pg in pages:
            ...
            for ch in chs:
                all_chunks.append(ch); metas.append({"source":os.path.basename(p),"page":pg["page"],"chunk":ch})
    if not all_chunks:
        return None, None
...
def retrieve_kb(query:str, kb_index, kb_meta, top_k=4):
    ...
    for _,i in scores[:top_k]:
        m=kb_meta[i]; out.append({"chunk":m["chunk"],"source":m["source"],"page":m["page"]})
    return out
...
def onboarding():
    if not st.session_state.onboarding_completed:
        with st.expander("🌟 처음 사용자 가이드", expanded=True):
            ...
            if st.button("🎯 시작하기"): st.session_state.onboarding_completed=True; st.rerun()
            ...
            with st.expander("🔑 OpenAI API 키 입력"):
                ...
                if st.button("저장"):
                    if api_key.startswith("sk-"):
                        st.session_state.openai_api_key=api_key; st.success("저장됨. Rerun 해주세요.")
                    else:
...
def page_today():
    ...
    if st.button("💝 분석하고 저장", type="primary"):
        ...
        if audio_val is not None:
            audio_bytes=audio_val.read(); audio_b64=base64.b64encode(audio_bytes).decode()
            with st.spinner("🎵 목소리 신호 계산 중..."):
                vf=extractor.extract(audio_bytes); update_baseline(vf)
            ...
            if openai_client and not diary_text:
                with st.spinner("🤖 음성 → 텍스트 전사 중..."):
                    ...
                    if tx: diary_text=tx; st.info(f"🎤 들은 이야기: {tx}")
                    else: st.warning("전사에 실패했습니다. 텍스트로 입력해 주세요.")
        if not diary_text:
            st.warning("텍스트를 입력하거나 음성을 녹음해 주세요."); return
        ...
        entry={"id":len(st.session_state.diary_entries)+1,"date":today_key(),"time":current_time(),
               "text":diary_text,"analysis":final,"audio_data":audio_b64,"mental_state":coach_card}
        ...
        with c1:
            ...
            st.subheader("💖 감정 (텍스트 기반)"); st.write(", ".join(final.get("emotions",[])))
            st.caption("감정 라벨은 텍스트만으로 판정합니다."); st.markdown("</div>", unsafe_allow_html=True)
        with c2:
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.subheader("📊 마음 상태")
        ...
        if voice_analysis:
            ...
            cues=final["voice_analysis"]["voice_cues"]; qtxt="높음" if cues["quality"]>0.7 else ("보통" if cues["quality"]>0.4 else "낮음")
            ...
            st.caption("※ 목소리 신호는 보조 지표이며, 감정은 텍스트 기반입니다.")
...
def page_dashboard():
    ...
    if not st.session_state.diary_entries: st.info("기록이 아직 없어요."); return
    st.subheader("📊 전체 통계")
    ...
    st.subheader("📋 상세 기록")
    df=pd.DataFrame([{"날짜":e["date"],"시간":e["time"],"감정":", ".join(e["analysis"].get("emotions",[])),
                     ...
                     "신뢰도":f"{e['analysis'].get('confidence',0.6):.2f}"} for e in st.session_state.diary_entries])
    c1,c2=st.columns(2)
...
def page_journey():
    st.header("시간에 따른 변화")
    if not st.session_state.diary_entries: st.info("기록이 쌓이면 추세를 보여드릴게요. 📈"); return
    c1,c2=st.columns(2)
    ...
    df=pd.DataFrame([{"날짜시간":f"{e['date']} {e['time']}","날짜":e['date'],
                      "스트레스":e["analysis"].get("stress_level",0),
                      "에너지":e["analysis"].get("energy_level",0),
                      "기분":e["analysis"].get("mood_score",0)+70} for e in entries])
    with c2: metric=st.selectbox("지표 선택",["전체","스트레스","에너지","기분"])
    if metric=="전체": st.line_chart(df.set_index("날짜시간")[["스트레스","에너지","기분"]])
    else:
        st.line_chart(df.set_index("날짜시간")[[metric]])
        if metric=="기분": st.caption("※ 시각화를 위해 +70 조정 (실제 -70~70)")
    st.subheader("📊 추세 분석")
...
def page_calendar():
    st.header("📅 감정 캘린더")
    if not st.session_state.diary_entries: st.info("기록이 쌓이면 캘린더로 볼 수 있어요!"); return
    today=kst_now()
    months=set([e["date"][:7] for e in st.session_state.diary_entries]); months.add(today.strftime("%Y-%m"))
    ...
    with c2: st.markdown(f"### {year}년 {month}월")
    month_entries={}
    for e in st.session_state.diary_entries:
        if e["date"].startswith(sel):
            day=int(e["date"].split("-")[2]); month_entries.setdefault(day,[]).append(e)
    ...
    for w_idx,week in enumerate(cal):
        ...
        for d_idx,day in enumerate(week):
            if day==0:
                cols[d_idx].markdown("<div style='height:60px;'></div>", unsafe_allow_html=True); continue
            entries=month_entries.get(day,[])
            is_today=(day==today.day and month==today.month and year==today.year)
            if entries:
                latest=entries[-1]; emos=latest.get("analysis",{}).get("emotions",[]); emoji=emotion_emoji(emos); color=emotion_color(emos)
                ...
                if clicked: st.session_state[f"show_day_{year}_{month}_{day}"]=True
            else:
                border="border:2px solid #667eea;" if is_today else "border:1px solid #ddd;"
            ...
            if entries:
                ...
                for i,e in enumerate(entries):
                    ...
                    with st.expander(f"📝 {e.get('time','')} - {emos}", expanded=(i==0)):
                        st.write(e.get("text",""))
                        a=e.get("analysis",{}); c1,c2,c3=st.columns(3)
...
def page_goals():
    ...
    with st.expander("➕ 새로운 목표 추가하기"):
        ...
        with c2:
            if gtype=="consistency": target=st.slider("주간 목표 기록 횟수",1,7,5); desc=f"일주일에 {target}번 이상 기록"
            elif gtype=="stress":   target=st.slider("목표 스트레스 (이하)",10,50,30); desc=f"스트레스 {target} 이하 유지"
            elif gtype=="energy":   target=st.slider("목표 에너지 (이상)",50,90,70); desc=f"에너지 {target} 이상 유지"
            else:                    target=st.slider("목표 기분 (이상)",0,50,20);  desc=f"기분 {target} 이상 유지"
    ...
    if not active: st.info("설정된 목표가 없습니다."); return
    st.subheader("📊 목표 진행 상황")
    for g in active:
        info=check_goal_progress(g); prog,cur,status=info["progress"],info["current_value"],info["status"]
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        c1,c2,c3=st.columns([3,1,1])
        with c1:
            st.write(f"**{g['description']}**"); st.progress(prog/100); st.caption(f"진행률: {prog:.1f}% | 현재값: {cur:.1f}")
        with c2: st.success(status) if status=="달성!" else st.info(status)
        with c3:
            if st.button("🗑️", key=f"del_goal_{g['id']}"): g["active"]=False; st.success("삭제됨"); st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
...
def page_voice():
    ...
    if not entries: st.info("음성 기록이 아직 없습니다."); return
    ...
    a,b=st.columns(2)
    with a:
        x1,x2=st.columns(2); x1.metric("피치 평균", f"{vf.get('pitch_mean',0):.1f} Hz"); x2.metric("피치 변동", f"{vf.get('pitch_variation',0):.3f}")
        x3,x4=st.columns(2); x3.metric("음성 에너지", f"{vf.get('energy_mean',0):.3f}"); x4.metric("최대 에너지", f"{vf.get('energy_max',0):.3f}")
    with b:
        y1,y2=st.columns(2); y1.metric("말하기 속도", f"{vf.get('tempo',0):.0f} BPM"); y2.metric("영교차율", f"{vf.get('zcr_mean',0):.3f}")
        y3,y4=st.columns(2); y3.metric("HNR(명료도)", f"{vf.get('hnr',0):.1f} dB"); y4.metric("Jitter(안정)", f"{vf.get('jitter',0):.4f}")
    if st.session_state.prosody_baseline:
        ...
        if st.button("베이스라인 초기화"): st.session_state.prosody_baseline={}; st.success("초기화 완료"); st.rerun()
    st.caption("※ 보조지표이며, 감정 라벨은 텍스트로 판단합니다.")

def page_archive():
    st.header("나의 이야기 아카이브")
    if not st.session_state.diary_entries: st.info("아직 기록이 없어요."); return
    ...
    st.subheader("🔍 기록 탐색")
    c1,c2,c3=st.columns(3)
    with c1: stext=st.text_input("🔍 텍스트 검색", placeholder="키워드…")
    with c2:
...
def page_kb():
    ...
    if st.session_state.kb_index is None:
        ...
        st.warning("GitHub에 PDF가 **LFS로 올라가 있거나**, Actions 배포 시 **git lfs pull**이 안 되면 파일이 0바이트/포인터일 수 있어요.")
        return
...
def generate_simple_weekly_report(entries: list[dict]) -> dict:
    ...
    if avg_stress < 40 and avg_energy > 60: trend = "개선됨"
    elif avg_stress > 70 or avg_energy < 30: trend = "주의필요"
    else: trend = "안정적"
    ...
    return {
        ...
        "patterns": {"best_days": [best_day.get("date", "")], "challenging_days": [worst_day.get("date", "")],
                     "emotional_patterns": f"이번 주는 전반적으로 {trend} 상태를 보였습니다."},
        "recommendations": {"priority_actions": ["스트레스 관리에 집중", "규칙적인 운동"] if avg_stress > 50 else ["현재 상태 유지"],
...
show_disclaimer()
if not st.session_state.show_disclaimer:
    if 'page' not in locals(): page="🎙️ 오늘의 이야기"
